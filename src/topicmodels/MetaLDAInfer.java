...
public class MetaLDAInfer implements Serializable {
    ...
    public double computePerplexityOnTestInstances(InstanceList instances, int numIterations, int thinning, int burnIn) {
        MLDouble testThetaMat = null;

        double perplexity = 0.0;

        testThetaMat = new MLDouble("test_theta", getInferredDistributions(instances, numIterations, thinning, burnIn, true));
        perplexity = computePerplexityForEverySecondWord(instances, testThetaMat.getArray(), phi);
